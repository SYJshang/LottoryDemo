...
@interface SYJKJGPController ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation SYJKJGPController
...
- (void)getHttpPic{
    ...
    [SYJHttpHelper GET:[NSString stringWithFormat:@"http://m.zhuoyicp.com/kaijang/gpkj?getData=1"] parameters:nil success:^(id responseObject) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SYJDFModel *model = arr[indexPath.row];
    SYJKJGPDetailVC *vc = [[SYJKJGPDetailVC alloc]init];
    vc.lotteryNo = [NSString stringWithFormat:@"%ld",model.lotId ];
    vc.time = model.prizeDate;
